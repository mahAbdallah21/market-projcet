...
@section('body')
    ...
    <h3 class="text-gray-700 text-3xl font-medium">{{__('message.products')}}</h3>
    ...
    <div class="flex flex-col mt-8">
        <div class="mt-4">
            <div class="flex rounded-md bg-white py-4 px-4 justify-end justify-items-end ">
                <button type="'button" class="px-6 py-3 bg-purple-400 rounded-md text-white font-medium tracking-wide hover:bg-purple-900 ml-3"><a href="{{route('products.create')}}">{{__('message.Add New product')}}</a></button>
                ...
                <table class="min-w-full">
                    <thead>
                        <tr>
                            ...
                            <th class="px-6 py-3 border-b border-gray-200 bg-gray-50">{{__('message.Action')}}</th>
                        </tr>
                    </thead>
                    ...
                    <tbody class="bg-white">
                        ...
                        <tr>
                            <td class="px-6 py-4 whitespace-no-wrap border-b border-gray-200 text-sm leading-5
                             text-gray-500">{{$key + $products->firstItem()}}</td>
                            ...
                            <td class="px-6 py-4 whitespace-no-wrap border-b border-gray-200">
                                ...
                                <img src="{{ asset('storage/'.$image)}}" alt="" width='100' height='70'>


                                @else
                                <div class="flex-shrink-0 h-10 w-10">
                                    <img class="h-10 w-10 rounded-full" src="https://gearbox.imgix.net/https%3A%2F%2Fstatic-a.imgix.net%2Fespresso.jpg?mask=ellipse&w=250&h=250&fm=png&auto=format&fit=crop&ixlib=js-2.0.0&s=4d7f04ea74ae759ebe57e8ee850dc7a4" alt="" />
                                </div>
                            ...
                            <td class="px-6 py-4 whitespace-no-wrap border-b border-gray-200 text-sm leading-5 text-gray-500">{{$product->category->name}}</td>
                            ...
                            <td class="px-8 py-6  whitespace-no-wrap text-right border-b border-gray-200 text-sm leading-5 font-medium">
                                ...
                                <a href="{{route('products.show' ,$product->id)}}" class="text-indigo-600 hover:text-indigo-900 p-3 ">Show</a>
                                <a href="{{route('products.edit' ,$product->id)}}" class="text-green-400 hover:text-green-900 p-3 ">Edit</a>
                        ...
                        @empty
                        <tr  class="px-6 py-4 whitespace-no-wrap border-b border-gray-200 text-sm leading-5 text-gray-500">
                            <td colspan='8' class="px-6 py-4 whitespace-no-wrap border-b border-gray-200 text-sm leading-5 text-gray-500">{{ __('message.No Result Yet') }}</td>
