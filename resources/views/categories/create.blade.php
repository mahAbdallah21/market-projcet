        ...
        <div class="p-4 bg-white rounded-md shadow-md">
            ...
            <form method="POST" enctype="multipart/form-data" action="{{route('categories.store')}}">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-6 mt-4">
                    ...
                    <div class="mt-4">
                        <label class="text-gray-700 @error('meta_title_ar') is-invalid @enderror"
                            for="meta_title_ar">{{__('message.Arabic META TITLE')}}</label>
                        <input class="form-input w-full mt-2 rounded-md focus:border-indigo-600" name="meta_title_ar"
                            value="{{old('meta_title_ar')}}" type="text">
                        @error('meta_title_ar')
                        <div
                            class="font-regular relative mb-4 block w-full rounded-lg bg-red-600 p-4 text-base leading-5 text-white opacity-100">
                            {{ $message }}</div>
                    ...
                    <div class="mt-4">
                        <label class="text-gray-700 @error('meta_title_en') is-invalid @enderror"
                            for="META_TITLE">{{__('message.English META TITLE')}}</label>
                        <input class="form-input w-full mt-2 rounded-md focus:border-indigo-600" name="meta_title_en"
                            value="{{old('meta_title_en')}}" type="text">
                        @error('meta_title_en')
                        <div
                            class="font-regular relative mb-4 block w-full rounded-lg bg-red-600 p-4 text-base leading-5 text-white opacity-100">
                            {{ $message }}</div>
                    ...
                    <div class="">
                        ...
                        <div class="relative inline-flex w-full self-center">
                            ...
                            <select name="category_id"
                                ...
                                @foreach (App\Models\category::orderBy('name')->whereNull('category_id')->pluck('name', 'id')->toArray() as $id =>
                                $name)
                    ...
                    <div class="mt-4 w-full bg-gray-50 rounded-lg border border-gray-200 ">
                        <div class="flex justify-between items-center py-2 px-3 border-b">
                            <div class="flex flex-wrap items-center divide-gray-200 sm:divide-x">
                                ...
                                <label for="editor" class="sr-only">{{__('message.Description')}}</label>
                            </div>
                        </div>
                        <div class="py-2 px-4 bg-white rounded-b-lg">

                            <textarea id="editor" name="meta_description" rows="8"
                                class="block px-0 w-full text-sm text-gray-800 bg-white border-0  focus:ring-0  "
                                placeholder="{{__('message.Description')}}"></textarea>
                        </div>
